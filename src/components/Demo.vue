<template>
  <div style="height: 100%; width: 100%">
    ...
    <div>
      <h3>简介</h3>
      <h4>{{ synopsis }}</h4>
      <span :style="if_like_colour()" @click="submit_like"
        ><i :class="if_like()"></i> {{ likes }}</span
      >
    </div>
    <el-divider></el-divider>
    <div>
    ...
    <div v-if="!loading_comments">
      <div
        v-for="(parent_comment, index) in parent_comments_list"
      ...
      >
        <el-divider content-position="left">
          <div>
            ...
            <el-avatar
              size="small"
              :src="parent_comment.comment_avatar_url"
        ...
        <div v-if="parent_comment.comment_id === comment.parent_comment_id">
          <div
            style="margin-left: 10%"
            v-for="comment in comments_list"
            :key="comment.comment_avatar_url"
          >
            <el-avatar
              size="small"
              :src="comment.comment_avatar_url"
        ...
        <el-divider content-position="right">
          <div style="height: 32px">
            <span>{{ parent_comment.create_time }}</span>
            <el-divider direction="vertical" v-if="authorized"></el-divider>
            <el-button
              type="text"
              @click="reply_comment(parent_comment)"
              size="small"
              v-if="authorized"
              >回复
            </el-button>
            <el-button
              type="text"
              @click="delete_comment(parent_comment)"
              size="small"
              v-if="user_id === parent_comment.user_id"
              >删除
            </el-button>
          </div>
        </el-divider>
    ...
    <div v-if="authorized">
      ...
      <el-button type="primary" native-type="submit" @click="post_comment"
        >评论</el-button
      >
    </div>
    <div v-else>
...
export default {
  ...
  methods: {
    ...
    if_like_colour() {
      if (this.like === false) {
        ...
        return "font-weight: bold; font-size: large; color: #409EFF; cursor:pointer";
    },
    cancel_rate() {
      let params = this.$route.query;
      params.rate = "0";
      this.$api.get("/physlet_api/postRate", { params }).then(() => {
  ...
  beforeMount() {
    ...
    this.$api.post("/physlet_api/getSim", params).then((response) => {
  ...
  mounted() {
    ...
    this.$api.get("/physlet_api/checkLogin").then((response) => {
      if (response.data.code === 200) {
        this.authorized = true;
        this.$api.get("/physlet_api/myInfo").then((response) => {
