<template>
  <div>
    <h2>编辑我的模拟</h2>
    <el-table stripe :data="simulation_to_edit" style="width: 100%">
      <el-table-column prop="simulation_name" label="名称"> </el-table-column>
      <el-table-column prop="simulation_category.name" label="类别">
      </el-table-column>
      <el-table-column prop="simulation_access" label="可见性">
      </el-table-column>
    </el-table>

    <el-select
      style="margin-top: 15px"
      v-model="category_changed"
      placeholder="请选择类别"
    >
      <el-option
    ...
    <el-button type="primary" native-type="submit" @click="save_change"
      >修改</el-button
    >
  </div>
</template>
...
export default {
  ...
  methods: {
    save_change() {
      ...
      this.$api.post("/physlet_api/editSimulation", params).then((response) => {
  ...
  mounted() {
    this.$api.get("/physlet_api/getCategories").then((response) => {
