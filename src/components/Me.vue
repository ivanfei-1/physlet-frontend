  ...
  <el-container>
    ...
    <el-main v-if="!loading_my_simulations">
      ...
      <el-card
        ...
        v-loading="loading_my_simulations"
      ...
      >
        <div slot="header">
          <div>
            ...
            <div style="float: right">
              <el-button size="small" @click.stop="open_edit_form(simulation)"
                >编辑</el-button
              >
              <el-popconfirm
                title="确定删除? "
                @confirm="delete_simulation(simulation)"
              >
          ...
          <el-button
            type="text"
            v-if="!loading_my_simulations"
            @click.stop="jump_to_my_simulation(simulation)"
            style="font-size: 23px; margin-left: 0"
          >
        ...
        <span
          ...
          >{{ simulation_access(simulation) }}</span
        >
        <br />
        <span
      ...
      <el-dialog
        title="编辑模拟"
        :visible.sync="edit_form_visibility"
        :center="true"
      >
        <el-form :inline="false" :model="edit_form">
          <el-form-item label="名称" style="width: 35%">
            <el-input placeholder="" v-model="edit_form.sname" clearable>
            </el-input>
          </el-form-item>
          <el-form-item label="类别">
            <el-select v-model="edit_form.cid" placeholder="类别">
              <el-option
                ...
                :value="category.edit_category_id"
              ></el-option>
            </el-select>
          </el-form-item>
    ...
    <el-aside>
      ...
      <el-form label-width="100px" style="width: 90%">
        ...
        <el-form-item>
          <el-button size="small" type="success" @click="submitFile"
            >点击上传</el-button
          >
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    jump_to_my_simulation(simulation) {
      this.$router.push({
        path: "/demo",
        query: { sid: simulation.simulation_id },
      });
    },
    edit_Simulation(row) {
      this.$router.push({
    ...
    submitFile() {
      ...
      this.$api
        .post("/physlet_api/uploadSim", formData, { headers })
        .then((res) => {
  ...
  mounted() {
    this.$api.post("/physlet_api/getMySims").then((response) => {
      let data = response.data.data;
      for (let syn = 0; syn < data.length; syn++) {
        let simulation_list = {};
        simulation_list.simulation_id = data[syn].sid;
    ...
    this.$api.get("/physlet_api/getCats").then((response) => {
      let data = response.data.data;
      for (let syn = 0; syn < data.length; syn++) {
        let category = {};
        let edit_category = {};
